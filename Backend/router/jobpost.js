...
router.post('/',async(request,response)=>{
    ...
    const {provider_id,job_title,job_description,location,category_id,experienece_required,salary,companyname,companyimage} = request.body

    try{
    const statement = 
                    `insert into jobs 
                            (provider_id,job_title,job_description,location,
                            category_id,experienece_required,salary,companyname,companyimage) 
                    values 
                            ( ?, ? , ?, ? , ? , ? , ? , ? , ?)`
    const result = await db.execute(
        statement,
        [provider_id,job_title,job_description,location,category_id,experienece_required,salary,companyname,companyimage]) 
